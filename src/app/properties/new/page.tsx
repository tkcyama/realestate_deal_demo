...
export default function NewPropertyPage() {
  ...
  async function save(data: FormData, status: 'draft' | 'pending_approval') {
    ...
    // Geocoding
    const address = [data.prefecture, data.city, data.town, data.address_detail]
      .filter(Boolean).join('')
    setGeocoding(true)
    ...
    const payload = {
      ...
      ncf: data.ncf ? Number(data.ncf) : null,
      noi_yield: data.noi && price ? Number(data.noi) / price : null,
      ncf_yield: data.ncf && price ? Number(data.ncf) / price : null,
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <form className="space-y-6">
        ...
        <Section title="財務情報">
          <div className="grid grid-cols-2 gap-4">
            <Field label="売却希望価格（円）" required error={errors.price?.message}>
              <Input type="number" placeholder="5000000000" {...register('price')} />
            </Field>
